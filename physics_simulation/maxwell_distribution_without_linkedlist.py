...
for i in range(N):
    ball[i]=sphere(pos=vector(5*random()+2,5*random()+2,5*random()+2),radius=0.05,color=color.green) #각 입자상태를 설정
    v[i]=vector(1000*(random()-0.5),1000*(random()-0.5),1000*(random()-0.5)) #각 입자의 속도상태를 설정
...
#lc[0],lc[1],lc[2]=20,20,20 #각 축을 나누는 개수 지정
#rc[0],rc[1],rc[2]=10/lc[0],10/lc[1],10/lc[2] #cell의 각 축 길이 지정
